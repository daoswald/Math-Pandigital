...
considered numeric digits when operating in base 16 mode.

Beware the possible traps and pitfalls.  Perl sees the integer, C<0123456789> as
C<123456789>, which may not be what you were expecting.  If there's a
possibility of a significant leading zero, be sure to pass a string of digits
...
Setting a base of 1 (unary), and a zero of false (zeros precluded) will cause
the constructor to throw an exception; it makes no sense to exclude the only
digit legal for unary.

The C<is_pandigital> test tries to disqualify a string of digits in the least
computationally expensive way possible first, and then moves to progressively
more demanding tests.  First length is tested, then a regex is used to
ensure only permissible digits occur, and finally a hash is used to count
occurrences of each digit, ensuring a minimum of one of each.
